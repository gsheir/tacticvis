...
class Competitions:
    ...
    def get_season(self, competition_name, season_name):
        condition = (self.competitions["competition_name"] == competition_name) & (
            self.competitions["season_name"] == season_name
        )
        season_row = self.competitions.loc[condition]

        assert (
            len(season_row) == 1
        ), f"Failed to get season, Statsbomb API should return 1 season, not {len(season_row)}"
        ...
        try:
            ...
            season.load_competition_events(
                season_row["country_name"].iloc[0],
                season_row["competition_name"].iloc[0],
                season_row["season_name"].iloc[0],
                season_row["competition_gender"].iloc[0],
            )
